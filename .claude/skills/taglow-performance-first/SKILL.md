...
For broad API/DB/analysis changes, use `.claude/agents/taglow-admin-performance-auditor.md` as a read-only reviewer. Give it changed files, expected data volume, and target flows. Ask it to report memory, network, normalization, query plan, index, RLS, caching, pagination, and API-shape risks.
